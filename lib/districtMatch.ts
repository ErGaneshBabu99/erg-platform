// ---------------------------------------------------------------------------
// Pure, framework-agnostic district ranking logic. No fetch, no Prisma, no
// DOM — safe to run on the server OR in the browser. This is the same
// algorithm that was validated in the API route, extracted so the client
// can rank all 77 districts locally, instantly, on every keystroke.
// ---------------------------------------------------------------------------
...
export interface RankedDistrict extends DistrictRow {
...
// District names are short (<20 chars), so even run against every candidate
// and several comparison windows per candidate, this stays well under 1ms
// for a 77-row list.
...
// `target` in the same order, but not necessarily contiguously. This is
// what makes abbreviation-style typing work — "ktm" isn't a typo of
// "Kathmandu" (edit distance is too high), it's letters picked out in
// order (K-a-T-h-M-a-n-d-u). Also catches consonant-skeleton typing like
// "pchthr" -> "Panchthar". Returns null if no match, otherwise how tight
// the match was (a smaller span between matched letters = a better match).
...
// Compares the query against several windows of the name (not just one
// same-length slice) so a missing/extra/swapped letter near the start
// doesn't throw off the whole comparison, and also compares against the
// FULL name so near-complete, near-correct spellings score well.
...
// Combines both fuzzy strategies into a single sortable score (lower =
// better match). A candidate qualifies for the fuzzy tier if EITHER
// strategy is confident — abbreviations pass via subsequence, typos pass
// via edit distance, and whichever fits better wins.
...
/**
 * Ranks districts against a query using Google-style tiered matching:
 * exact > startsWith > word-startsWith > contains > fuzzy (typo/abbreviation).
 * Pure and synchronous — safe to call on every keystroke.
 */
export function rankDistricts(districts: DistrictRow[], rawQuery: string, limit = 8): RankedDistrict[] {
  ...
  for (const d of districts) {
    ...
    // (or another searchable field) are ever added.
