...
export default function PrevNextNav({ prev, next }: PrevNextNavProps) {
  ...
  return (
    <nav className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-12 pt-10 border-t border-white/10">
      {/* Previous */}
      <div>
        {prev ? (
          <Link
            href={`/blog/${prev.slug}`}
            className="group flex flex-col gap-2 p-5 rounded-2xl bg-white/5 border border-white/10 hover:border-white/20 hover:bg-white/8 transition-all duration-300 h-full"
          >
      ...
      <div>
        {next ? (
          <Link
            href={`/blog/${next.slug}`}
            className="group flex flex-col gap-2 p-5 rounded-2xl bg-white/5 border border-white/10 hover:border-white/20 hover:bg-white/8 transition-all duration-300 text-right h-full"
          >
